...
export const update_tiles = (tile_path: string, start: number, end: number, stored_hash: Uint8Array) => {
  if (end - start !== 32) {
    // this hash was an intermediate of the tile
    // so it will never be persisted
    return null
  }
  let tile_data = read_tile(tile_path)
  ...
  global_tiles[tile_path] = tile_data
  return tile_data
}
