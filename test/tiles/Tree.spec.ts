...
import { Hash, Tree, VerifyInclusion, VerifyConsistency, toHex, prettyInclusionProof, prettyConsistencyProof, prettyProof } from "../../src";
...
it('sanity', () => {
  ...
  const r0 = tree.hash()
  expect(toHex(r0)).toBe('395aa064aa4c29f7010acfe3f25db9485bbd4b91897b6ad7ad547639252b4d56')
  tree.appendData(tree.encodeData('L789'))
  const r1 = tree.hash()
  expect(toHex(r1)).toBe('1798faa3eb85affab608a28cf885a24a13af4ec794fe3abec046f21b7a799bec')
  ...
  const r2 = tree.hash()
  expect(toHex(r2)).toBe('3322c85256086aa0e1984dff85eab5f1e11d4b8fbbd6c4510611e3bbab0e132a')
...
describe('TestTreeIncremental', () => {
  ...
  it('the empty tree hash', () => {
    expect(toHex(tree.hash())).toBe('e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855')
  })
  ...
  it('the empty leaf hash', () => {
    ...
    expect(toHex(tree.hash())).toBe('6e340b9cffb37a989ca544e6bb780a2c78901d3fb33738768511a30617afa01d')
  })

  it('the first intermediate / root', () => {
    const leaf = tree.encodeData(`entry-1`)
    const leafHash = th.hashLeaf(leaf)
    expect(toHex(leafHash)).toBe(`e868811a482c27d50b6d45dde79c465d6adb9b06645100477a90cf3d8518898b`)
    tree.appendHash(leafHash)
    expect(toHex(tree.hash())).toBe('ef2154dde385935cccbaa4129960d8fb571ae75737203e70252c47431d0b2e3e')
  })

  it('the first unbalanced root', () => {
    tree.appendData(tree.encodeData(`entry-${2}`))
    expect(toHex(tree.hash())).toBe('74d80c0d37c9d19475c2c3e6d3183ebd39031191bbd16571e2db0e4a5f999c4f')
  })

  it('the second unbalanced root', () => {
    tree.appendData(tree.encodeData(`entry-${3}`))
    expect(toHex(tree.hash())).toBe('c34777e9c093d8a7b9907621cf12f7c868a1beb196d09a866f36933849832705')
  })
})