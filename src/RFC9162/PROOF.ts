...
const SUBPROOF = async (m: number, D_n: Uint8Array[], b: boolean): Promise<Uint8Array[]> => {
  ...
  if (m < n) {
    ...
    if (m <= k) {
      ...
      const second = await MTH(CUT(D_n, k, n))
      ...
      const second = await MTH(CUT(D_n, 0, k))
      return first.concat(second)
    }
  }
...
export const PROOF = async (m: number, D_n: Uint8Array[]): Promise<Uint8Array[]> => {
